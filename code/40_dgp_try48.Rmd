...
all_df_gamma <- all_df_gamma_w_fails %>% filter(!is.na(cover_ind))

all_df_gamma_fails     <- all_df_gamma_w_fails %>% filter(is.na(cover_ind))
...
all_df_att <- all_df %>%
  ...
  mutate(
    nobs      = as.numeric(as.character(n)) * 2,
    abs_bias  = abs(bias),
    n         = as.factor(n),
    prop_extreme    = as.factor(prop_extreme),
    magnitude_level = as.factor(magnitude_level),
