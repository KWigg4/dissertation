...
# --- Save failure log CSV (one row per distinct failure, with error messages) ---
failure_log <- all_df %>%
  filter(failed == TRUE) %>%
...
# --- Save human-readable failure summary txt (for dissertation reference) ---
log_path <- here::here(paste0("results/failure_summary_log_", dt, ".txt"))

sink(log_path)
cat(sprintf("Failure summary — run date: %s\n", dt))
cat(sprintf("Total failed replications (gamma model): %d / %d\n\n",
            sum(all_df$model == "gamma" & all_df$failed == TRUE, na.rm = TRUE),
            sum(all_df$model == "gamma")))

all_df %>%
  filter(model == "gamma", failed == TRUE) %>%
  count(n, prop_extreme, magnitude_level, treatment, gamma_error, sort = TRUE) %>%
  print(n = Inf)
sink()
...
message(sprintf("\nTotal failed replications (gamma model): %d / %d",
                sum(all_df$model == "gamma" & all_df$failed == TRUE, na.rm = TRUE),
                sum(all_df$model == "gamma")))
message("Failure summary written to: ", log_path)
...
#                   <1 = model underestimates SE (anti-conservative), >1 = overestimates (conservative)
