...
aov_bias <- aov(bias ~ treatment * n * prop_extreme * magnitude_level, 
  data = all_df_gamma)
car::Anova(aov_bias, type=3)
effectsize::eta_squared(aov_bias, partial=TRUE)
...
aov_bias <- aov(emp_se ~ treatment * n * prop_extreme * magnitude_level, 
  data = all_df_gamma)
car::Anova(aov_bias, type=3)
effectsize::eta_squared(aov_bias, partial=TRUE)
...
aov_cov_t3_pes_replicate <- aov_type3_pes(
  cover_ind ~ treatment * n * prop_extreme * magnitude_level,
  data = all_df_gamma
  )

fn_save_results(aov_cov_t3_pes_replicate)
...
# ATT across condition
aov_att_est_t3 <- aov_type3_pes(
  # use est? 
  est ~ treatment * n * prop_extreme * magnitude_level, 
  data = all_df_att
) |> 
  # remove partial eta-squared that's in the function for the other outcomes not this one. 
  select(-pes)
fn_save_results(aov_att_est_t3)
...
aov_att_bias_t3 <- aov_type3_pes(
  ...
  # don't report pes for this, it's just calculated in the function for others
  select(-pes)

fn_save_results(aov_att_bias_t3)

# trying on summ_att to see if SS are better / if even the same??;  
aov_att_bias_t3_summ <- aov_type3_pes(
  bias ~ treatment * n * prop_extreme * magnitude_level, 
  data =summ_att
) |> 
  # don't report pes for this, it's just calculated in the function for others
...
fn_save_results(aov_att_bias_t3_summ)
