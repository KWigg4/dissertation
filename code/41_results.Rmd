...
p_zoomed <- ggplot(
  ...
  labs(title = "A) Zoomed View", x = "η²", y = "Effect") +
  theme_minimal() +
  theme(legend.position = "bottom")
...
p_full <- ggplot(
  ...
  geom_vline(xintercept = 0.14, linetype = "dashed", color = "darkgreen", 
             alpha = 0.7) +
  annotate("text", x = 0.01, y = 0.5, label = "Small", size = 3, color = "red",
           vjust = 0, hjust = -0.1) +
  annotate("text", x = 0.06, y = 0.5, label = "Med",   size = 3, color = "orange", 
           vjust = 0, hjust = -0.1) +
  annotate("text", x = 0.14, y = 0.5, label = "Large", size = 3, color = "darkgreen",
           vjust = 0, hjust = -0.1) +
  scale_fill_manual(
    values = c("p < .001" = "steelblue", 
               "p < .01"  = "cornflowerblue",
  ...
  scale_x_continuous(limits = c(0, 0.20), breaks = seq(0, 0.15, 0.03)) +
  labs(title = "B) Full Scale (with benchmarks)", x = "η²", y = NULL) +
  theme_minimal() +
  theme(
...
p_combined <- p_zoomed + p_full +
  ...
  plot_layout(ncol = 2, nrow = 2, guides = "collect",
              ...
              heights = c(1, 0.05)) +
  plot_annotation(
    title    = "Eta-Squared for Significant Effects: Gamma Model Bias",
    subtitle = "All effects are trivial - even the largest (0.003) is far below the 'small' benchmark (0.01)"
  )
