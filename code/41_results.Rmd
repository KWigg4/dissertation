...
compute_ESE_metrics <- function(df) {
  ...
  df |>
    ...
    summarise(
      ...
      # MORRIS p 2086
      # # >1=overcov, <1=undercov
      ratio_modse_emp = se_model_mean / ese,
      # % by which model SE misses EmpSE from MORRIS article per Tsai's request
...
ese_by_treatment <- ese_results |>
  ...
  summarise(
    ...
    # Morris calibration metrics (averaged across the 27 scenarios per treatment)
    mean_ratio_modse_emp = mean(ratio_modse_emp),   
      # avg ModSE/EmpSE; <1 = underest → undercoverage
    ...
    mean_rel_error_modse = mean(rel_error_modse),   
      # avg % error; negative = underest → undercoverage
    ...
    sd_rel_error_modse   = sd(rel_error_modse),     
      # how consistent is the miscalibration?
    n_scen_underest      = sum(ratio_modse_emp < 1),  
      # how many of 27 scenarios underestimate SE
    prop_scen_underest   = mean(ratio_modse_emp < 1),
    
    # Previous calibration flag
...
write.csv(
  tab_cov_treatment,
  here::here("results/tab_cov_treatment_2026-02-23.csv")
)
...
# Dot plot for means by scenario: 
