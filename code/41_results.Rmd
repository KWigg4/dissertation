...
```{r}
# ── Overall (N = 67,371 rows)
tab_cov_overall <- all_df_gamma |>
  summarise(
    n_reps        = n(),
    mean_cov      = mean(cover_ind, na.rm = TRUE),
    sd_cov        = sd(cover_ind, na.rm = TRUE),
    median_cov    = median(cover_ind, na.rm = TRUE),
    min_cov       = min(cover_ind, na.rm = TRUE),
    max_cov       = max(cover_ind, na.rm = TRUE),
    n_covered     = sum(cover_ind == 1, na.rm = TRUE),
    n_not_covered = sum(cover_ind == 0, na.rm = TRUE)
  )

# ── By treatment only
tab_cov_treatment <- all_df_gamma |>
  group_by(treatment_fct) |>
  summarise(
    n_reps        = n(),
    mean_cov      = mean(cover_ind, na.rm = TRUE),
...
# ── Scenario-level: collapse to one rate per scenario first 
# Step 1: one coverage rate per scenario (proportion of 500 reps covered)
cov_by_scenario <- all_df_gamma |>
  group_by(treatment_fct, nobs_fct, prop_extreme_fct, magnitude_fct) |>
  summarise(
    n_reps    = n(),
    coverage  = mean(cover_ind, na.rm = TRUE),   # proportion covered in this scenario
    .groups = "drop"
  )
# N = 135 rows (5 treatments × 27 scenarios)

# Step 2: descriptives across those 135 scenario-level rates
...
# (This is just cov_by_scenario with a flag added; use it directly for heatmaps etc.)

# Step 3: treatment-level summary ACROSS the 27 scenarios per treatment
tab_cov_passfail <- cov_by_scenario |>
  group_by(treatment_fct) |>
  summarise(
    n_scenarios   = n(),                          # should be 27
...
tab_cov_treatment <- all_df_gamma |>
  group_by(treatment_fct) |>
  mutate(coverage=mean(cover_ind)) |> 
  summarise(
    scenarios = dplyr::n(),
...
# Pass/fail table
tab_cov_passfail <- tab_cov_scenario |>
  group_by(treatment_fct) |>
  summarise(
    scenarios = dplyr::n(),
    n_pass = sum(coverage >= 0.95),
...
model_att <- aov(
  abs_bias ~ treatment_fct * nobs_fct * prop_extreme_fct * magnitude_fct, 
  data = all_df_att)
