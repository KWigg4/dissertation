...
make_lolly_panel <- function(
  ...
  p +
    scale_color_manual(name = tier_name, 
                       values = tier_colors, 
                       drop = FALSE) +
...
```{r}
dat_combined <- readRDS(here::here("results/example_datasets.RDS"))
# write.csv(dat_combined,
#          here::here("results/dat_combined.csv"))

dat_example1     <- readRDS(here::here("results/example_dataset1.RDS"))

dat_example1 |> 
  group_by(ind_tx) |> 
  summarize(mean = mean(TOTEXPYY),
            sd = sd(TOTEXPYY))
...
dat_example1 |> 
  gtsummary::tbl_summary(
    include = c(ind_gt0, HISPANX, ind_female, ind_post),
    by = ind_tx,
    type = list(
      c(ind_gt0, HISPANX, ind_female, ind_post) ~ "categorical"
    ),
    digits = list(
      c(ind_gt0, HISPANX, ind_female, ind_post) ~ c(0, 2)
    )
  ) |> 
  add_overall()
...
# ── Data prep 
# dat_example1 in chunk above needed in this chunk 
zoom_cutoff      <- quantile(dat_example1$TOTEXPYY, 0.95)
bulk_cut         <- 50000
dat_combined_pos <- dat_combined |> filter(TOTEXPYY > 0)
dat_extremes     <- dat_combined |> filter(TOTEXPYY > zoom_cutoff)

# ── Right panel from plot_twopanel: jitter of extremes by prop_extreme ────────
dat_extremes_stats <- dat_extremes %>%
  group_by(prop_extreme_lbl) %>%
  summarise(
    mean_val   = mean(TOTEXPYY, na.rm = TRUE),
    median_val = median(TOTEXPYY, na.rm = TRUE)
  )

p_extremes <- ggplot(
  dat_extremes,
  aes(x = prop_extreme_lbl, y = TOTEXPYY, color = prop_extreme_lbl)
) +
  geom_jitter(width = 0.2, size = 1.2, alpha = 0.4) +
  geom_crossbar(
    data      = dat_extremes_stats,
    aes(y = mean_val, ymin = mean_val, ymax = mean_val),
    width     = 0.4, color = "black", linewidth = 0.4, linetype = "solid", fatten = 0
  ) +
  geom_crossbar(
    data      = dat_extremes_stats,
    aes(y = median_val, ymin = median_val, ymax = median_val),
    width     = 0.4, color = "black", linewidth = 0.4, linetype = "dashed", fatten = 0
  ) +
  scale_y_continuous(labels = scales::dollar_format(scale = 1e-6, suffix = "M")) +
  scale_color_manual(
    values = c("1% Extreme" = "#4E9ABE", "5% Extreme" = "#F4A261", "10% Extreme" = "#E76F51"),
    guide  = "none"
  ) +
  labs(
    title    = "A) Jitter of Extreme Values",
    subtitle = NULL,
    caption  = NULL,
    x        = NULL,
    y        = "Total Expenditure"
...
# ── Right panels from plot_hist_sixpanel: histograms of extremes by row ───────
get_max_bin_count <- function(prop_lbl) {
  dat <- dat_combined_pos |> filter(prop_extreme_lbl == prop_lbl, TOTEXPYY > bulk_cut)
  h   <- ggplot_build(ggplot(dat, aes(x = TOTEXPYY)) + geom_histogram(bins = 60))
  max(h$data[[1]]$count, na.rm = TRUE)
}
right_max_count <- get_max_bin_count("10% Extreme")

make_hist_right <- function(prop_lbl, show_x, show_y, title = NULL) {
  fill_col <- c(
    "1% Extreme"  = "#4E9ABE",
    "5% Extreme"  = "#F4A261",
    "10% Extreme" = "#E76F51"
  ...
  dat        <- dat_combined_pos |> filter(prop_extreme_lbl == prop_lbl, TOTEXPYY > bulk_cut)
  mean_val   <- mean(dat$TOTEXPYY,   na.rm = TRUE)
  median_val <- median(dat$TOTEXPYY, na.rm = TRUE)

  ggplot(dat, aes(x = TOTEXPYY)) +
    geom_histogram(bins = 60, fill = fill_col, color = "white", linewidth = 0.2) +
    geom_vline(xintercept = mean_val,   color = "black", linewidth = 0.4, linetype = "solid") +
    geom_vline(xintercept = median_val, color = "black", linewidth = 0.4, linetype = "dashed") +
    scale_x_continuous(labels = scales::dollar_format(scale = 1e-6, suffix = "M")) +
    scale_y_continuous(labels = scales::comma, limits = c(0, right_max_count * 1.05)) +
    labs(
      title = title,
      x     = if (show_x) "Expenditure (> $50K)" else NULL,
...
p_1_right  <- make_hist_right(
  "1% Extreme",  show_x = FALSE, show_y = TRUE,  title = "B) Histograms of Extreme Values")
p_5_right  <- make_hist_right(
  "5% Extreme",  show_x = FALSE, show_y = TRUE,  title = NULL)
p_10_right <- make_hist_right(
  "10% Extreme", show_x = TRUE,  show_y = TRUE,  title = NULL)

# ── Combine ───────────────────────────────────────────────────────────────────
plot_combined <- patchwork::wrap_plots(
  p_extremes,
  patchwork::wrap_plots(p_1_right, p_5_right, p_10_right, nrow = 3),
  ncol   = 2,
  widths = c(1, 1)
) +
  patchwork::plot_annotation(
    title    = "Extreme Expenditures by Proportion of Extreme Values",
    subtitle = glue::glue(
      "Values above 95th percentile ({scales::dollar(zoom_cutoff, scale=1e-3, suffix='K')}); ",
      "Magnitude level = 1 ($50K–$1M); n = 5,000\n",
      "\u2014 Mean (solid)  - - - Median (dashed)"
    ),
    caption  = "Note: Zero-expenditure observations excluded. Right column y-axis fixed to 10% extreme maximum for comparability.",
    theme    = theme(
...
ggsave(
  here::here(paste0("figures/extremes_combined_", dt, ".png")),
  plot_combined,
  width  = 11,
  height = 9,
  dpi    = 300
...
tbl_bias_overall <- all_df_gamma %>%
  dplyr::summarise(
    ...
    abs_mean_bias    = mean(abs(bias), na.rm=TRUE),
...
Decided not to keep, the next one is better.  
...
```{r plot_absbias_dist}
plot_gamma_absbias <- all_df_gamma |>
  mutate(
    Magnitude = factor(Magnitude),
    Treatment = factor(Treatment, levels = rev(c(
      "No treatment",
  ...
  mutate(dist_from_zero = abs(est)) |>
  ggplot(aes(x = abs_bias, y = Treatment, color = dist_from_zero)) +
  geom_jitter(height = 0.2, size = 1.2, alpha = 0.4) +
  stat_summary(
    fun = mean, geom = "point", shape = "|",
    size = 5, color = "goldenrod"
  ...
  labs(
    title    = "Distribution of Gamma Interaction Absolute Bias by Treatment and Scenario",
    subtitle = "Each point = one replication; gold bar = mean",
    x        = "Estimate",
    y        = "Data treatment method"
  ) +
...
ggsave(
  here::here(paste0("figures/gamma_est_distribution_", dt, ".png")),
  plot_absbias_dist, width = 14, height = 20, dpi = 300
)
```

### ANOVA gamma abs bias  
...
combined_table <- type3_df %>%
  ...
  select(Term, Df, `Sum Sq`, `F value`, `Pr(>F)`, Eta2, Interp_Eta2, Eta2_partial, Interp_Eta2_partial) %>%
...
write.csv(combined_table, 
          here::here(paste0("results/aov_gamma_absbias_", dt, ".csv")),
          row.names = FALSE)
...
p_full <- ggplot(
  ...
  annotate(
    "text", x = 0.01, y = 1, 
    label = "Small", size = 3, color = "red",    
    hjust = -0.1, vjust = -0.3) +
  annotate(
    "text", x = 0.06, y = 1, 
    label = "Med",   size = 3, color = "orange",  
    hjust = -0.1, vjust = -0.3) +
  annotate(
    "text", x = 0.14, y = 1, 
    label = "Large", size = 3, color = "#1a7a1a", 
    hjust = -0.1, vjust = -0.3) +
  scale_fill_manual(
  ...
  labs(
    ...
    subtitle = "Treatment has the largest effect (η² = 0.1815, Large); all effects significant at p < .001",
    x        = "η²",
    y        = "Effect"
  ) +
  theme_minimal() +
...
ggsave(
  here::here(paste0("figures/effect_sizes_gamma_absbias_", dt, ".png")),
  p_full_att, width = 10, height = 7, dpi = 300
)

```    

```{r g_absbias_sigplots}
# ── Shared color scale and theme ──────────────────────────────────────────────
trt_colors <- c(
  "No treatment"                = "#0072B2",
  "Top-coding"                  = "#E69F00",
  "Mean-preserved top-coding"   = "#009E73",
...
# ── 3-way summaries ─
means_trt_nobs_prop <- all_df_gamma |>
  group_by(Treatment, N, Proportion) |>
  summarise(
    mean_abs_bias = mean(abs_bias, na.rm = TRUE),
    se_bias       = sd(abs_bias, na.rm = TRUE) / sqrt(n()),
    .groups       = "drop"
  )

means_trt_nobs_mag <- all_df_gamma |>
  group_by(Treatment, N, Magnitude) |>
  summarise(
    mean_abs_bias = mean(abs_bias, na.rm = TRUE),
    se_bias       = sd(abs_bias, na.rm = TRUE) / sqrt(n()),
    .groups       = "drop"
...
n_labeller <- labeller(N = function(x) paste0("N = ", x))

# ── Treatment × N × Proportion ─
plot_3way_trt_nobs_prop <- means_trt_nobs_prop |>
  ggplot(aes(x = Proportion, y = mean_abs_bias,
             group = Treatment, color = Treatment)) +
  geom_hline(yintercept = 0, linetype = "dashed",
             color = "grey30", linewidth = 0.4) +
  geom_line(linewidth = 0.9) +
  geom_errorbar(
    aes(ymin = mean_abs_bias - 1.96*se_bias,
        ymax = mean_abs_bias + 1.96*se_bias),
    width = 0.15, linewidth = 0.6
  ) +
  geom_point(size = 3.5, shape = 18) +
  facet_wrap(~ N, nrow = 1, labeller = n_labeller)+
  scale_color_manual(name = "Treatment", values = trt_colors) +
  labs(
    title    = "Gamma Interaction Absolute Bias: Treatment × N × Proportion Extreme",
    subtitle = "Type III 3-way interaction. Means ± 95% CI. Facets = sample size.",
...
# ── Treatment × N × Magnitude ────────────────────────────────────────────────
plot_3way_trt_nobs_mag <- means_trt_nobs_mag |>
  ggplot(aes(x = Magnitude, y = mean_abs_bias,
             group = Treatment, color = Treatment)) +
  geom_hline(yintercept = 0, linetype = "dashed",
             color = "grey30", linewidth = 0.4) +
  geom_line(linewidth = 0.9) +
  ...
  scale_color_manual(name = "Treatment", values = trt_colors) +
  labs(
    title    = "Gamma Interaction Absolute Bias: Treatment × N × Magnitude",
    subtitle = "Type III 3-way interaction. Means ± 95% CI. Facets = sample size.",
    x        = "Magnitude level",
    y        = "Mean absolute bias"
  ) +
...
plot_3way_trt_nobs_prop
plot_3way_trt_nobs_mag

ggsave(here::here(paste0("figures/anova_3way_trt_nobs_prop_", dt, ".png")),
       plot_3way_trt_nobs_prop, width = 13, height = 5, dpi = 300)
ggsave(here::here(paste0("figures/anova_3way_trt_nobs_mag_", dt, ".png")),
       plot_3way_trt_nobs_mag, width = 13, height = 5, dpi = 300)
```  
...
ese_subtitle <- "X-axis: scenario ID 1-27 (see companion table). Filled dots = empirical SE (coloured by threshold); grey + = mean model SE."
...
cov_subtitle <- "X-axis: scenario ID 1–27 (see Appendix B). Dashed line = nominal 95%. <span style='color:#2ca02c'>**Green = at/above 95%**</span>; <span style='color:#d62728'>**Red = below 95%**</span>."
...
plot_cov_3row <- cowplot::plot_grid(
  cowplot::plot_grid(
    cowplot::ggdraw() +
      cowplot::draw_label(cov_title, fontface = "bold", size = 13, hjust = 0.5),
    ggplot2::ggplot() +
...
ggsave(
  here::here(paste0("figures/gamma_coverage_3row_", dt, ".png")),
  plot_cov_3row,
  width  = 11,
  height = 9,
  dpi    = 300
...
# Build ATT scenario-level summary (like prep_plot_bias_over_under) 
prep_att_bias <- all_df_att |>
  group_by(Treatment, N, Proportion, Magnitude) |>
  summarise(
    bias       = mean(bias, na.rm = TRUE),
    under_rate = mean(est < true, na.rm = TRUE),
...
# --- ATT-specific scenario key ---
scenario_key_att <- prep_att_bias |>
  mutate(
    N          = as.character(N),
  ...
  distinct(N, Proportion, Magnitude) |>
  mutate(
    nobs_num = as.numeric(gsub(",", "", N)),
    prop_num = as.numeric(Proportion),
    mag_num  = as.numeric(Magnitude)
  ) |>
  arrange(nobs_num, prop_num, mag_num) |>
  mutate(scenario_id = row_number()) |>
  select(scenario_id, N, Proportion, Magnitude)
...
# --- Attach scenario IDs and tiers ---
att_bias_plot_data <- prep_att_bias |>
  left_join(scenario_key_att, by = c("N", "Proportion", "Magnitude")) |>
  mutate(
    scenario_id = factor(scenario_id, levels = scenario_id_levels),
    Treatment   = factor(Treatment,   levels = treatment_order_3row),
    bias_tier   = case_when(
    ...
    bias_tier = factor(bias_tier, levels = c("<=|25|", ">|25|", ">|50|"))
  )

# --- ATT bias axis limits ---
att_x_min <- min(att_bias_plot_data$bias, na.rm = TRUE)
att_x_max <- max(att_bias_plot_data$bias, na.rm = TRUE)
att_bias_y_lims <- c(att_x_min, att_x_max)
...
make_att_bias <- function(trt, show_x, show_y) {
  make_lolly_panel(
    data        = att_bias_plot_data |> dplyr::filter(Treatment == trt),
    yvar        = bias,
    tier_var    = bias_tier,
    y_limits    = att_bias_y_lims,
    tier_colors = att_bias_colors,
    tier_name   = "|Bias| threshold",
    panel_title = trt,
    show_x_axis = show_x,
    show_y_axis = show_y,
    y_label     = "Mean ATT Bias"
  )
}

# --- 5 panels ---
p_ab1 <- make_att_bias("No treatment",               show_x = FALSE, show_y = TRUE)
p_ab2 <- make_att_bias("Mean-preserved top-coding",  show_x = FALSE, show_y = TRUE)
p_ab3 <- make_att_bias("Median-preserved top-coding",show_x = FALSE, show_y = FALSE)
p_ab4 <- make_att_bias("Top-coding",                 show_x = TRUE,  show_y = TRUE)
p_ab5 <- make_att_bias("Truncation",                 show_x = TRUE,  show_y = FALSE)
...
# --- Titles ---
att_bias_title    <- "ATT: Scenario-Level Mean Bias by Treatment"
att_bias_subtitle <- "X-axis: scenario ID 1–27 (see companion table).\nDashed line = zero bias. <span style='color:steelblue'>**Blue = |bias| \u226425**</span>; <span style='color:orange'>**Orange = |bias| >25**</span>; <span style='color:red'>**Red = |bias| >50**</span>."

x_label_att_bias <- cowplot::ggdraw() +
  cowplot::draw_label("Scenario ID", size = 10, hjust = 0.5)

plot_att_bias_3row <- cowplot::plot_grid(
  cowplot::plot_grid(
    cowplot::ggdraw() +
      cowplot::draw_label(
        att_bias_title,
    ...
    ggplot2::ggplot() +
      ggtext::geom_richtext(
        aes(x = 0.5, y = 0.5, label = att_bias_subtitle),
        size        = 3.5,
        color       = "grey40",
        fill        = NA,
        label.color = NA,
...
ggsave(
  here::here(paste0("figures/att_bias_lollipop_3row_", dt, ".png")),
  plot_att_bias_3row, width = 13, height = 13, dpi = 300
)
```


```{r}
plot_att_bias_dist <- all_df_att |>
  mutate(
    treatment_fct = factor(Treatment, levels = rev(c(
      "No treatment",
      "Top-coding",
      "Mean-preserved top-coding",
      "Median-preserved top-coding",
  ...
  mutate(dist_from_zero = abs(bias)) |>
  ggplot(aes(x = bias, y = treatment_fct, color = dist_from_zero)) +
  geom_jitter(height = 0.2, size = 1.2, alpha = 0.4) +
  stat_summary(
    fun = mean, geom = "point", shape = "|",
    size = 3, color = "goldenrod"
  ) +
  # geom_vline(xintercept = 0, linetype = "dashed", color = "grey30") +
  scale_color_gradient(low = "steelblue", high = "red", name = "Distance from 0") +
  facet_grid(N + Magnitude ~ Proportion,
  ...
  labs(
    title    = "Distribution of ATT Bias by Data Treatment and Condition",
    subtitle = "Each point = one replication; gold bar = mean",
    x        = "Bias (ATT hat - ATT true)",
    y        = "Data treatment method"
  ) +
  theme_minimal(base_size = 10) +
  theme(
...
ggsave(
  here::here(paste0("figures/att_bias_distribution_", dt, ".png")),
  plot_att_bias_dist, width = 14, height = 20, dpi = 300
)
```  

```{r att_absbias_dist}

plot_att_absbias_dist <- all_df_att |>
  mutate(
    treatment_fct = factor(Treatment, levels = rev(c(
      "No treatment",
      "Top-coding",
  ...
  labs(
    title    = "Distribution of ATT Absolute Bias by Data Treatment and Condition",
    subtitle = "Each point = one replication; gold bar = mean",
    x        = "Absolute Bias (|ATT hat - ATT true|)",
    y        = "Data treatment method"
  ) +
  theme_minimal(base_size = 10) +
  theme(
...
plot_att_absbias_dist

ggsave(
  here::here(paste0("figures/att_absbias_distribution_", dt, ".png")),
  plot_att_absbias_dist, width = 14, height = 20, dpi = 300
)
```
...
## FULL-FACTORIAL ANOVA ATT ABS BIAS  
...
```{r}
# error bars didn't show up, so took them off. 

library(ggrepel)
library(ggtext)

# Plot 1: Treatment x Proportion
twoway_trt_prop <- all_df_att |>
...
prop_labels <- twoway_trt_prop |>
  group_by(Treatment) |>
  slice_tail(n = 1)

plot_trt_prop <- ggplot(
  twoway_trt_prop,
  aes(
    x     = Proportion,
    y     = mean_abs_bias,
  ...
  geom_text(
    data        = prop_labels,
    aes(x = 3.12, label = Treatment),
    hjust       = 0,
    size        = 3.2,
    show.legend = FALSE
  ) +
  scale_x_discrete(expand = expansion(add = c(0.2, 1.0))) +
  scale_y_continuous(
  ...
  coord_cartesian(clip = "off") +
  labs(
    title = "ATT Absolute Bias: *Treatment*:*Proportion*",
    x     = "Proportion of extreme values",
    y     = "Mean absolute bias"
  ) +
  base_theme +
...
ggsave(
  here::here(paste0("figures/att_absbias_2way_trt_prop_", dt, ".png")),
  plot_trt_prop,
  width  = 10,
  height = 5.5,
  dpi    = 300
...
# Plot 2: Treatment x Magnitude
twoway_trt_mag <- all_df_att |>
  group_by(Treatment, Magnitude) |>
  summarise(
    mean_abs_bias = mean(abs_bias, na.rm = TRUE),
    .groups       = "drop"
  ) |>
...
plot_trt_mag <- ggplot(
  ...
  coord_cartesian(clip = "off") +
  labs(
    title = "ATT Absolute Bias: *Treatment*:*Magnitude*",
    x     = "Magnitude level",
    y     = "Mean absolute bias"
  ) +
  base_theme +
...
ggsave(
  here::here(paste0("figures/att_absbias_2way_trt_mag_", dt, ".png")),
  plot_trt_mag,
  width  = 10,
  height = 5.5,
  dpi    = 300
...
plot_combined <- plot_trt_prop / plot_trt_mag

plot_combined

ggsave(
  here::here(paste0("figures/att_absbias_2way_combined_", dt, ".png")),
  plot_combined,
  width  = 10,
  height = 10,
  dpi    = 300
