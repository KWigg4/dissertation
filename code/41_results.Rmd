...
# Can't do this on all_df and then subset there because that doesn't have cover_ind for some reason. Whatever. 
...
all_df_att <- readRDS(here::here("results/all_df_att_2026-02-09.RDS")) %>% 
  ...
  # mutate(
  #   across(c(est, se, bias), ~round(.x, 4))) |> 
  select(-c(se, d_est, d_true, d_bias, n, gamma_shape)) |> 
  # just reorder and remove n
  select(model, est, true, bias, 
         nobs, nobs_fct, 
         prop_extreme, prop_extreme_fct,
         magnitude_level, magnitude_fct,
         treatment, treatment_fct,
...
design_elements <- readRDS(here::here("results/summ_gamma_2026-02-09.RDS")) |> 
  select(
    n, nobs, prop_extreme, magnitude_level, treatment, design_id, n_design) |> 
...
summ_grid  <- readRDS(here::here("results/summ_grid_2026-02-09.RDS")) |> 
  ...
  select(design_id, n_design, model, 
         ...
         treatment, treatment_fct,
         everything()) |> 
  select(-c(d_mean, d_bias, d_emp_se, n))
...
tab_bias_all_descr <- all_df_gamma %>%
  dplyr::summarise(
    scenarios        = dplyr::n(),
...
# model_lm<-lm(bias ~ treatment * n * prop_extreme * magnitude_level,
#              data = all_df_gamma)

# trying it with summ_gamma?
model2 <- aov(bias ~ treatment * n * prop_extreme * magnitude_level, 
             data = summ_gamma)
model2_aov_t3 <- car::Anova(model2, type=3)
...
type3_anova_lm <- car::Anova(model_lm, type = "III") # same as aov above
print(type3_anova)
...
eta2_regular <- effectsize::eta_squared(model, 
                                        partial = FALSE,
                                        ci = 0.95)
# tried this with lm() model output above and it's the same. 
print(eta2_regular)
...
p_zoomed <- ggplot(plot_data, aes(x = Eta2_partial, y = Term, fill = Significant)) +
  geom_col() +
  scale_fill_manual(
...
p_full <- ggplot(plot_data, aes(x = Eta2_partial, y = Term, fill = Significant)) +
  geom_col() +
  ...
  geom_vline(xintercept = 0.14, linetype = "dashed", color = "green", alpha = 0.7) +
  annotate("text", x = 0.01, y = 6.5, label = "Small", size = 3, color = "red") +
  annotate("text", x = 0.06, y = 6.5, label = "Med", size = 3, color = "orange") +
  annotate("text", x = 0.14, y = 6.5, label = "Large", size = 3, color = "green") +
  scale_fill_manual(
    values = c("p < .001" = "steelblue", 
               "p < .01" = "cornflowerblue",
               "p < .05" = "lightblue"),
