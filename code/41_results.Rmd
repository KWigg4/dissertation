...
under_over_scen <- all_df_gamma %>%
  ...
  ungroup() |> 
  select(treatment, n_design, nobs, prop_extreme, magnitude_level, pct_under, pct_over, under_rate, over_rate) |> 
  mutate(
...
under_over_scen |> 
  arrange(nobs, pct_extreme, magnitude_level, treatment) |> 
  mutate(
    treatment = factor(
      treatment,
...
summ_gamma2 <- summ_gamma |> 
  mutate( treatment = as.character(treatment), 
          magnitude_level = as.numeric(magnitude_level) ) 
under_over_scen2 <- under_over_scen |> 
  mutate( treatment = as.character(treatment), 
          magnitude_level = as.numeric(magnitude_level) ) # Now join 
tab_consistency <- summ_gamma2 |> left_join( under_over_scen2, by = c( "treatment" = "treatment", "nobs" = "nobs", "prop_extreme" = "pct_extreme", "magnitude_level" = "magnitude_level" ) )
