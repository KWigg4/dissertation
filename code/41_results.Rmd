...
make_lolly_panel <- function(
    data,
    yvar,           # unquoted: bias or ese
    tier_var,       # unquoted: bias_tier or ese_tier
    y_limits,
    tier_colors,    # named colour vector
    tier_name,      # legend title
    panel_title,
    show_x_axis,    # TRUE only for bottom-row panels
    show_y_axis,    # TRUE only for left-column panels
    y_label = "",
    hline = 0,      # reference line (0 for bias and ESE)
    yvar2 = NULL    # optional: unquoted col for a second grey dot series
) {
  ...
  # Add second series (model SE) as grey dots if supplied
  if (!is.null(substitute(yvar2))) {
    p <- p + geom_point(aes(y = {{ yvar2 }}),
                        shape = 1, size = 1.8,
...
prep_plot_bias_over_under <- prep_plot_bias_over_under |>
  mutate(
    scenario_short = glue::glue("p={Proportion} n={N} m={Magnitude}")
  )
...
```{r}
plot_est_dist2 <- all_df_gamma |>
  mutate(Magnitude = factor(Magnitude)) |>
  ggplot(aes(x = est, y = Treatment)) +
  geom_jitter(height = 0.2, size = 0.3, alpha = 0.2, color = "steelblue") +
  stat_summary(
  ...
  labs(
    ...
    subtitle = "Each point = one replication; gold bar = mean; dashed line = true value (0)",
    x        = "Estimate",
    y        = "Treatment"
  ) +
  theme_minimal(base_size = 10) +
...
ese_subtitle <- "X-axis: scenario ID 1-27 (see companion table). Filled dots = empirical SE (coloured by threshold); open grey circles = mean model SE."
...
prep_plot_cov_ordered <- prep_plot_cov |>
  mutate(
    ...
    Treatment  = factor(Treatment,  levels = treatment_order_cov),
...
plot_cov_heat2 <- tab_cov_scenario %>%
  dplyr::mutate(
    cell = glue::glue("p={Proportion}  mag={Magnitude}  n={N}"),
