...
# Extract shared legend from a version with legend enabled
shared_bias_legend <- (
  p_b1 + theme(legend.position = "bottom",
...
bias_patch <- (
  ...
  plot_annotation(
    title    = "Gamma Interaction: Scenario-Level Mean Bias by Treatment",
    subtitle = "X-axis: scenario ID 1-27 (see companion table). Dashed line = zero bias.",
    theme = theme(
...
x_label_bias <- cowplot::ggdraw() +
  cowplot::draw_label("Scenario ID", fontsize = 10, hjust = 0.5)
...
```{r descr_empse, eval=FALSE, include=FALSE}

compute_ESE_metrics <- function(df, group_vars = c("Treatment","N","Proportion","Magnitude")) {
...
prep_plot_ese_ordered <- prep_plot_ese |>
  mutate(
    ...
    Treatment  = factor(Treatment,  levels = treatment_order_ese),
    ese_tier = case_when(
      ese > 0.50 ~ ">0.50",
      ese > 0.25 ~ ">0.25",
      TRUE       ~ "≤0.25"
