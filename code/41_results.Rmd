...
```{r plot_ex_opt1}
plot_boxplot_extremes <- ggplot(
  dat_combined,
  aes(x = prop_extreme_lbl, y = TOTEXPYY, 
      fill = prop_extreme_lbl, color = prop_extreme_lbl)
) +
  geom_boxplot(
    outlier.shape  = 16,
    outlier.size   = 0.8,
    outlier.alpha  = 0.4,
    width          = 0.5,
    color          = "grey30",   # fixed outline color so boxes don't blend in
    linewidth      = 0.5
  ) +
  ...
  labs(
    title    = "Distribution of Total Expenditures by Proportion of Extreme Values",
    subtitle = "Magnitude level = 1 ($50,000–$1,000,000); Sample size = 5,000; outlier dots show extreme values",
...
plot_boxplot_extremes

ggsave(
  here::here(paste0("figures/boxplot_extremes_", dt, ".png")),
  plot_boxplot_extremes,
...
# Left panel: zoomed in on bulk of distribution
p_zoom <- ggplot(
  dat_combined,
  aes(x = prop_extreme_lbl, y = TOTEXPYY, fill = prop_extreme_lbl)
) +
  geom_violin(
    trim  = FALSE,
    alpha = 0.8,
  ...
  labs(
    title    = "A) Bulk of Distribution",
    subtitle = "Zoomed to $0–$5K",
    x        = NULL,
    y        = "Total Expenditure"
  ) +
  theme_minimal(base_size = 13) +
...
# Compute a zoom cutoff: just below where extremes start (q95 of prop=0.01 condition)
dat_example1 <- readRDS(here::here("results/example_dataset1.RDS"))
zoom_cutoff <- quantile(dat_example1$TOTEXPYY, 0.95)

# --- Left panel: zoomed boxplot of bulk ---
p_bulk <- ggplot(
  dat_combined,
  aes(x = prop_extreme_lbl, y = TOTEXPYY, fill = prop_extreme_lbl)
) +
  geom_boxplot(
    outlier.shape = NA,   # suppress outliers — shown separately on right
    width         = 0.5,
    color         = "grey30",
    linewidth     = 0.5
  ) +
  coord_cartesian(ylim = c(0, zoom_cutoff)) +
  scale_y_continuous(labels = scales::dollar_format(scale = 1e-3, suffix = "K")) +
  scale_fill_manual(
    values = c(
      "1% Extreme"  = "#4E9ABE",
  ...
  labs(
    title    = "A) Bulk of Distribution",
    subtitle = glue::glue("Zoomed to ≤ {scales::dollar(zoom_cutoff, scale=1e-3, suffix='K')} \n(95th percentile of example 1)"),
    x        = NULL,
    y        = "Total Expenditure"
  ) +
  theme_minimal(base_size = 13) +
...
# --- Right panel: only extreme values ---
# --- Precompute mean and median for extremes panel ---
...
p_extremes <- ggplot(
  ...
  labs(
    title    = "B) Extreme Values Only",
    subtitle = glue::glue("Values above 95th percentile ({scales::dollar(zoom_cutoff, scale=1e-3, suffix='K')})"),
    caption  = "— Mean  - - Median",
    x        = NULL,
    y        = NULL
  ) +
  ...
  theme(
    plot.title         = element_text(face = "bold"),
    plot.subtitle      = element_text(color = "grey40"),
    plot.caption       = element_text(color = "grey30", hjust = 0.5),
    panel.grid.major.x = element_blank()
  )

# --- Combine ---
plot_twopanel <- patchwork::wrap_plots(p_bulk, p_extremes, ncol = 2, widths = c(1.2, 1)) +
  plot_annotation(
    title    = "Distribution of Total Expenditures by Proportion of Extreme Values",
    subtitle = "Magnitude level = 1 ($50K–$1M); n = 5,000",
    caption  = "Note: Panel A shows the bulk of the distribution (below 95th percentile).\nPanel B shows only values above the 95th percentile, illustrating how the extreme tail grows with proportion.",
    theme    = theme(
...
ggsave(
  here::here(paste0("figures/twopanel_extremes_", dt, ".png")),
  plot_twopanel,
...
# Add mean and median lines to right side only: 
make_hist_panel <- function(
    prop_lbl, col, show_x, show_y, title = NULL, y_limit = NULL) {
  ...
  # Add mean and median lines for right panels only
  if (col == "right") {
...
# --- ATT-specific scenario key (same structure, built from all_df_att) ---
...
# Should be 27
nrow(scenario_key_att)

# Then join with characters on both sides
...
shared_att_bias_legend <- gtable::gtable_filter(
  ggplot2::ggplotGrob(tmp_att_bias), "guide-box"
)
...
att_bias_subtitle <- "X-axis: scenario ID 1–27. Dashed line = zero bias."
...
plot_att_bias_3row <- cowplot::plot_grid(
  ...
  att_bias_patch,
  cowplot::plot_grid(
    x_label_att_bias, shared_att_bias_legend,
    ncol = 1, rel_heights = c(0.35, 0.65)
  ),
  ncol = 1, rel_heights = c(0.1, 1, 0.15)
)
