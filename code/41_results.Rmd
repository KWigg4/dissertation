...
# 2: Type III Sum of Squares ANOVA Table

type3_anova <- car::Anova(model, type = "III")
print(type3_anova)

# 3: Effect Sizes (eta sq and partial eta sq)

eta2_regular <- effectsize::eta_squared(model, partial = FALSE, ci = 0.95)
eta2_partial <- effectsize::eta_squared(model, partial = TRUE,  ci = 0.95)
print(eta2_partial)

# 4: Comb. Table (get Type III SS + Effect Sizes)
...
eta2_reg_df <- eta2_regular %>%
  as.data.frame() %>%
  select(Parameter, Eta2, CI_low, CI_high) %>%
  rename(Eta2_CI_low = CI_low, Eta2_CI_high = CI_high)
...
combined_table_full <- type3_df %>%
  ...
  select(Term, Df, `Sum Sq`, `F value`, `Pr(>F)`, 
         ...
         Eta2_partial, Eta2_partial_CI_low, Eta2_partial_CI_high) %>%
  arrange(desc(`F value`))

print(combined_table_full, digits = 4)

# combine simple (point estimates only - could b better for publication??)
combined_table <- type3_df %>%
  left_join(eta2_reg_df %>% select(Parameter, Eta2), 
            by = c("Term" = "Parameter")) %>%
  left_join(eta2_part_df %>% select(Parameter, Eta2_partial), 
            by = c("Term" = "Parameter")) %>%
  mutate(
    ...
    Eta2 = round(Eta2, 4),
    Eta2_partial = round(Eta2_partial, 4)
  ) %>%
  select(Term, Df, `Sum Sq`, `F value`, `Pr(>F)`, Eta2, Eta2_partial) %>%
  arrange(desc(`F value`))
...
# full table with CIs
write.csv(combined_table_full, 
          file = paste0("results/anova_type3_full_", dt, ".csv"),
          row.names = FALSE)

# simplified table
write.csv(combined_table, 
          file = paste0("results/anova_type3_simple_", dt, ".csv"),
          row.names = FALSE)


# 6: publication-ready tbl (HTML/PDF output? )
#============================================================

combined_table %>%
  mutate(
    `95% CI (η²)` = paste0("[", round(Eta2_CI_low, 3), ", ", 
                           round(Eta2_CI_high, 3), "]"),
    `95% CI (η²p)` = paste0("[", round(Eta2_partial_CI_low, 3), ", ", 
                            round(Eta2_partial_CI_high, 3), "]")
  ) %>%
  select(Term, Df, `F value`, `Pr(>F)`, Eta2, Eta2_partial) %>%
  kable(
    caption = "Type III ANOVA with Effect Sizes: Gamma Model Bias",
    digits = c(0, 0, 3, 4, 4, 4),
    col.names = c("Effect", "df", "F", "p", "η²", "η²p"),
    align = c("l", "r", "r", "r", "r", "r")
  ) %>%
  kable_styling(
  ...
  footnote(
    general = "η² = eta-squared; η²p = partial eta-squared. Effect size benchmarks: small (0.01), medium (0.06), large (0.14).",
    general_title = "Note:",
    footnote_as_chunk = TRUE
  )


# 7: Identify Significant Effects and Large Effect Sizes
...
large_effects <- combined_table %>%
  filter(Eta2_partial >= 0.06) %>%  # Medium to large effects
  arrange(desc(Eta2_partial))

cat("\n=== MEDIUM TO LARGE EFFECT SIZES (η²p ≥ .06) ===\n")
print(large_effects)
...
cat("\n=== SUMMARY ===\n")
cat("Total effects tested:", nrow(combined_table), "\n")
cat("Significant at p < .001:", sum(grepl("<.001", combined_table$`Pr(>F)`)), "\n")
cat("Significant at p < .05:", sum(as.numeric(gsub("<.001", "0", combined_table$`Pr(>F)`)) < 0.05), "\n")
cat("Medium+ effect sizes (η²p ≥ .06):", sum(combined_table$Eta2_partial >= 0.06, na.rm = TRUE), "\n")
cat("Largest partial eta²:", round(max(combined_table$Eta2_partial, na.rm = TRUE), 4), 
    "for", combined_table$Term[which.max(combined_table$Eta2_partial)], "\n")


# opt: post-hoc tests for significant main effects? Need?

# for pairwise comparisons for significant effects:
library(emmeans)

# ex: Post-hoc for treatment effect
if ("treatment" %in% significant_effects$Term) {
...
# ex: Post-hoc for magnitude_level (if significant)
if ("magnitude_level" %in% significant_effects$Term) {
...
# OPTIONAL: check ANOVA assumptions ? Need?

cat("\n=== CHECKING ANOVA ASSUMPTIONS ===\n")

# 1. Normality of residuals
shapiro_test <- shapiro.test(residuals(model)[1:5000])  
  # Shapiro limited to 5000 obs
...
# 3. Diagnostic plots
png(paste0("figures/anova_diagnostics_", dt, ".png"), width = 10, height = 8, 
    units = "in", res = 300)
par(mfrow = c(2, 2))
plot(model)
dev.off()
