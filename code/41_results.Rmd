...
write.csv(tbl_bias_overall, 
          here::here("results/tbl_bias_overall_2026-02-22.csv"))
...
write.csv(tbl_bias_treatment, 
          here::here("results/tbl_bias_treatment_2026-02-23.csv"))
...
write.csv(tbl_bias_scen, 
          here::here("results/tbl_bias_scen_2026-02-23.csv"))
...
# ---- ese_by_treatment: aggregate 135 scenario cells → 5 treatment rows ----
...
write.csv(
  tab_cov_treatment,
  here::here("results/tab_cov_treatment_2026-02-24.csv")
)

# coverage by scenarios (N=135)
...
# --- Step 0: build scenario_short ---
prep_plot_cov <- summ_gamma |>
  mutate(
    scenario_short = glue::glue("p={prop_extreme_fct} n={nobs_fct} m={magnitude_fct}")
  )

# --- Step 1: order scenarios by coverage (worst to best) ---
...
# --- Step 2: order treatments by frequency of coverage < 0.95 (worst to best) ---
treatment_order_cov <- prep_plot_cov |>
  group_by(treatment_fct) |>
  summarise(n_below = sum(coverage < 0.95, na.rm = TRUE), .groups = "drop") |>
  arrange(desc(n_below)) |>
  pull(treatment_fct)

# --- Step 3: apply ordering and coverage flag ---
prep_plot_cov_ordered <- prep_plot_cov |>
  mutate(
    ...
    cov_flag       = ifelse(coverage < 0.95, "Below 95%", "At/Above 95%"),
...
plot_cov_facet <- prep_plot_cov_ordered |>
  ...
  geom_segment(
    aes(x = scenario_short, xend = scenario_short,
        y = 0.95, yend = coverage, color = cov_flag),
    linewidth = 0.5
  ) +
  geom_point(
    aes(color = cov_flag),
  ...
  scale_color_manual(
    ...
    values = c(
      "At/Above 95%" = "#2ca02c",   # green
      "Below 95%"    = "#d62728"    # red
    )
  ) +
  scale_y_continuous(
    limits = c(
      min(0.85, min(prep_plot_cov_ordered$coverage, na.rm = TRUE) - 0.01),
      1.0
    ),
  ...
  labs(
    title    = "Gamma Interaction: Confidence Interval Coverage by Treatment and Scenario",
    subtitle = "Dashed line = nominal 95%; scenarios ordered worst to best within each panel; treatments ordered worst to best left to right",
    x        = NULL,
    y        = "Coverage"
  ) +
  theme_minimal(base_size = 10) +
...
plot_cov_facet
ggsave(here::here(paste0("figures/gamma_coverage_lollipop_facet_", dt, ".png")),
...
cov_min <- floor(min(summ_gamma$coverage, na.rm = TRUE) * 100) / 100
cov_max <- ceiling(max(summ_gamma$coverage, na.rm = TRUE) * 100) / 100

plot_cov_heat2 <- summ_gamma %>%
  dplyr::mutate(
    treatment_fct = forcats::fct_relevel(
      treatment_fct,
      "No treatment", "Top-coding",
...
att_est_overall <- summ_grid_att |>
  summarise(
    n_scen        = n(),
    mean_est_mean   = mean(mean_est, na.rm = TRUE),
    mean_est_sd     = sd(mean_est, na.rm = TRUE),
    mean_est_median = median(mean_est, na.rm = TRUE),
    mean_est_mad    = mad(mean_est, na.rm = TRUE),
    mean_est_min    = min(mean_est, na.rm = TRUE),
    mean_est_max    = max(mean_est, na.rm = TRUE),
    mean_true       = mean(mean_true, na.rm = TRUE),  # should be ~158 for all
    .groups = "drop"
  )
fn_save_results(att_est_overall)

# --- Table 1: mean_est descriptives by treatment ---
att_est_by_treatment <- summ_grid_att |>
  group_by(treatment_fct) |>
  summarise(
    n_scen        = n(),
    mean_est_mean   = mean(mean_est, na.rm = TRUE),
...
gt_att_est <- att_est_by_treatment |>
  mutate(across(where(is.numeric), fmt)) |>
  gt() |>
  tab_header(
    title    = "ATT: Descriptives of Mean Estimate by Treatment",
    subtitle = "Each row averages across 27 scenarios; true ATT ≈ 158"
  ) |>
  cols_label(
    treatment_fct   = "Treatment",
    n_scen          = "N scenarios",
    mean_est_mean   = "Mean",
    mean_est_sd     = "SD",
    mean_est_median = "Median",
    mean_est_mad    = "MAD",
    mean_est_min    = "Min",
    mean_est_max    = "Max",
    mean_true       = "Mean true ATT"
  ) |>
...
att_bias_overall <- summ_grid_att |>
  summarise(
    n_scen          = n(),
    mean_bias       = mean(bias, na.rm = TRUE),
    sd_bias         = sd(bias, na.rm = TRUE),
...
att_bias_by_treatment <- summ_grid_att |>
  group_by(treatment_fct) |>
  summarise(
    n_scen          = n(),
    mean_bias       = mean(bias, na.rm = TRUE),
...
gt_att_bias <- att_bias_by_treatment |>
  mutate(
    across(c(mean_bias, sd_bias, median_bias, mad_bias,
             min_bias, max_bias, mean_abs_bias, median_abs_bias), fmt),
    prop_neg_bias = fmt_pct(prop_neg_bias)
  ) |>
  gt() |>
  tab_header(
    title    = "ATT: Descriptives of Bias by Treatment",
    subtitle = "Bias = mean_est − true ATT; each row averages across 27 scenarios"
  ) |>
  cols_label(
    treatment_fct   = "Treatment",
    n_scen          = "N scenarios",
    ...
    n_neg_bias      = "N scen negative",
    prop_neg_bias   = "% scen negative"
  ) |>
  tab_options(data_row.padding = px(2))
...
# Save both
gtsave(gt_att_est,  here::here("results/tbl_att_est_by_treatment.rtf"))
gtsave(gt_att_bias, here::here("results/tbl_att_bias_by_treatment.rtf"))

```  

```{r plot_att_bias}
# --- Step 0: build scenario_short ---
prep_plot_att_bias <- summ_grid_att |>
  mutate(
    scenario_short = glue::glue("p={prop_extreme_fct} n={nobs_fct} m={magnitude_fct}")
  )

# --- Step 1: order scenarios by frequency of |bias| > 50 across all treatments ---
scenario_order_att <- prep_plot_att_bias |>
  group_by(scenario_short) |>
  summarise(n_large_bias = sum(abs(bias) > 50, na.rm = TRUE), .groups = "drop") |>
  arrange(desc(n_large_bias)) |>
  pull(scenario_short)

# --- Step 2: order treatments by frequency of |bias| > 100 (worst to best) ---
treatment_order_att <- prep_plot_att_bias |>
  group_by(treatment_fct) |>
  summarise(n_large_bias = sum(abs(bias) > 100, na.rm = TRUE), .groups = "drop") |>
  arrange(desc(n_large_bias)) |>
  pull(treatment_fct)

# --- Step 3: apply ordering and bias tiers ---
prep_plot_att_bias_ordered <- prep_plot_att_bias |>
  mutate(
    scenario_short = factor(scenario_short, levels = scenario_order_att),
    treatment_fct  = factor(treatment_fct,  levels = treatment_order_att),
    bias_tier = case_when(
      abs(bias) > 500  ~ ">|500|",
      abs(bias) > 100  ~ ">|100|",
      TRUE             ~ "≤|100|"
    ),
    bias_tier = factor(bias_tier, levels = c("≤|100|", ">|100|", ">|500|"))
  )

att_y_min <- min(prep_plot_att_bias_ordered$bias, na.rm = TRUE)
att_y_max <- max(prep_plot_att_bias_ordered$bias, na.rm = TRUE)

# --- Step 4: plot ---
plot_att_bias_facet <- prep_plot_att_bias_ordered |>
  ggplot(aes(x = scenario_short, y = bias)) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "grey30", linewidth = 0.4) +
  geom_segment(
    aes(x = scenario_short, xend = scenario_short, y = 0, yend = bias),
  ...
  scale_y_continuous(limits = c(att_y_min, att_y_max)) +
  facet_wrap(
    ~ treatment_fct, nrow = 2,
    labeller = labeller(treatment_fct = label_wrap_gen(width = 15))
  ) +
  labs(
    title    = "ATT: Scenario-Level Bias by Treatment",
    subtitle = "Scenarios ordered by frequency of |bias| > 50; treatments ordered worst to best (left to right, top to bottom)",
    x        = NULL,
    y        = "Bias (ATT scale)"
  ) +
  theme_minimal(base_size = 10) +
...
plot_att_bias_facet
ggsave(here::here(paste0("figures/att_bias_lollipop_facet_", dt, ".png")),
       plot_att_bias_facet, width = 16, height = 8, dpi = 300)
