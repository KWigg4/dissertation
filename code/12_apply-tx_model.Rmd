...
did <- function(sim_ds, dv, sim_list) {
  ...
  formula_part1 <- ind_gt0 ~ ind_tx*ind_post + ind_tx + ind_post + ind_female + HISPANX + PCS42 + ind_inscov
  formula_part2 <- as.formula(paste0(dv, " ~ ind_tx*ind_post + ind_tx + ind_post+ ind_female + HISPANX + PCS42 + ind_inscov"))
...
# This approach aligns with the simulation setup, which doesn’t include an interaction in the dgf.
...
did_v2 <- function(sim_ds, dv) {
  ...
  formula_part2 <- as.formula(paste0(
    dv, " ~ ind_tx + ind_post + ind_female + HISPANX + PCS42 + ind_inscov"
  ))
