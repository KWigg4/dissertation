...
# to get intercept, coefficients for the data-generating function (so you don't have to justify choice)

fn_emp_model <- function(){
  # Part 1: Fit the logistic model for the probability of a non-zero outcome
  model_part1 <- glm(
    formula = ind_gt0 ~ ind_tx*ind_post + ind_tx + ind_post + ind_female + HISPANX + zPCS42 + INSCOVYY,
    data = fyc,
  ...
  model_part2 <- glm(
    formula = TOTEXPYY ~ ind_tx*ind_post + ind_tx + ind_post+ ind_female + HISPANX + zPCS42 + INSCOVYY,
    data = sim_positive,
  ...
  saveRDS(model_part1$coef_part1, here::here("data_processed/emp_coef_p1.RDS"))
  saveRDS(model_part1$coef_part2, here::here("data_processed/emp_coef_p2.RDS"))
...
```{r}
library(broom)
tidy(emp_model$model_part2)  |> 
  mutate(across(where(is.numeric), ~round(.x, 4)))
```
 
```{r}

fyc_wide <- reshape(fyc,
                   timevar = "ind_post",
                   idvar = "DUPERSID",
                   direction = "wide")

# Step 2: Compute correlation
cor(fyc_wide$PCS42.0, fyc_wide$PCS42.1, use = "complete.obs") #0.425434

# correlation for MCS42: 
cor(fyc_wide$MCS42.0, fyc_wide$MCS42.1, use="complete.obs")   # 0.4049928
...
simulate_panel_data <- function(
  ...
  rho = 0.4,                # pre/post corr for PCS42
  ...
  lp_cov <- emp_model$model_part1$coef_part1["(Intercept)"] + 
      emp_model$model_part1$coef_part1["ind_post"]*ind_post + 
      emp_model$model_part1$coef_part1["ind_tx"]*ind_tx + 
      emp_model$model_part1$coef_part1["ind_tx:ind_post"]*(ind_post*ind_tx) +
      emp_model$model_part1$coef_part1["ind_female"]*ind_female +
      emp_model$model_part1$coef_part1["HISPANX"]*HISPANX + 
      emp_model$model_part1$coef_part1["zPCS42"]*zPCS + 
      emp_model$model_part1$coef_part1["INSCOVYY"]*INSCOV
  ...
  #            0.2*ind_female - 0.1*HISPANX + u
    lp_cont <-  emp_model$model_part1$coef_part1["(Intercept)"] +  
    emp_model$model_part2$coef_part2["ind_post"]*ind_post + 
    emp_model$model_part2$coef_part2["ind_tx"]*ind_tx + 
    emp_model$model_part2$coef_part2["ind_tx:ind_post"]*(ind_tx*ind_post) + 
    emp_model$model_part2$coef_part2["ind_female"]*ind_female + 
    emp_model$model_part2$coef_part2["HISPANX"]*HISPANX + 
    emp_model$model_part2$coef_part2["zPCS42"]*zPCS  + 
    emp_model$model_part2$coef_part1["INSCOVYY"]*INSCOV
...
#lp_cont <- 7 + 0.4*ind_post + 0.6*ind_tx + 0.15*zPCS + 0.2*ind_female - 0.1*HISPANX
 ...
 model_part1 <- glm(
  ...
  # Part 2: Fit the Gamma model for the positive outcomes only
  sim_positive <- sim1 %>% filter(ind_gt0 == 1)
...
```{r compare_lr}
# coefficients used for simulation: 
emp_model$model_part1$coefficients
```  

```{r}
# coefficients from model ran on sim1: 
model_part1$coefficients
```
