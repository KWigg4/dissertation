...
simulate_panel_data <- function(
  ...
  lp_cov <- 0.5*ind_post + 0.7*ind_tx + 1.2*(ind_post*ind_tx) + 0.2*ind_female - 0.2*HISPANX + 0.1*zPCS + u
  ...
  lp_cont <- 7 + 0.4*ind_post + 0.6*ind_tx + 1.0*(ind_post*ind_tx) + 0.15*zPCS + 
...
## KW updating to make a binary for INSCOV 
final_sim_data <- final_sim_data |> 
  # 3 is not insured, 1 is any private, 2 is any commercial
  mutate(ind_inscov = ifelse(INSCOV==3, 0, 1))
