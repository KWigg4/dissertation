...
cor(fyc_wide$PCS42.0, fyc_wide$PCS42.1, use = "complete.obs")

# correlation for MCS42: 
cor(fyc_wide$MCS42.0, fyc_wide$MCS42.1, use="complete.obs")
```


```{r}
# Updates:
# -- 09/05 KW added INSCOV to models
# -- 09/05: Using MCS42 instead of INSCOV I think... 
...
simulate_panel_data <- function(
  ...
  # Inject extremes only if toggle is TRUE
  if (inject_extremes && n_ext > 0) {
    rng <- extreme_range(magnitude_level)
    TOTEXPYY[idx_ext] <- rlogunif(n_ext, rng[1], rng[2])
  }
