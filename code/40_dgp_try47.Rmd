...
```  
######  IF CLEAN IN 41_results.Rmd, remove below this line #################


##4 FULL-FACTORIAL ANOVA (RQ answers)

```{r fns_aov_type3}
tidy_aov_type3 <- function(model) {
  # Needed packages
  ...
  # car::Anova usually gives: "Sum Sq", "Df", "F value", "Pr(>F)"
  # Compute Mean Sq
  tab$`Mean Sq` <- tab$`Sum Sq` / tab$Df

  # Identify residual/error row (often "Residuals" or "(Residuals)")
  error_row <- grepl("Residual", tab$term, ignore.case = TRUE)
  ss_error  <- tab$`Sum Sq`[error_row]

  # Compute partial eta squared: SS_effect / (SS_effect + SS_error)
  tab$pes <- ifelse(
    !error_row,
  ...
  tab_out <- tab |>
    dplyr::mutate(
      ...
      # p-values: 3 decimals with <.001 rule
      ...
      # partial eta squared: 3 decimals
      pes = ifelse(!is.na(pes), round(pes, 3), NA_real_)
    ) |>
...
tidy_aov_type3_v2 <- function(formula, data) {
  # Needed packages
  if (!requireNamespace("car", quietly = TRUE)) {
    stop("Please install the 'car' package for Type III SS.")
  }
...
aov_bias_v1 <- tidy_aov_type3(aov_gamma_bias)

aov_bias_v2 <- tidy_aov_type3_v2(
...
aov_bias_t3 <- tidy_aov_type3(aov_gamma_bias)

write.csv(
  aov_bias_t3,
  here::here(paste0("results/aov_bias_t3_", dt, ".csv")))
...
eta_squared(aov_gamma_emp_se, partial=TRUE, type=3) # for partial eta squared (or general?)

tidy_aov_empse_t3 <- tidy_aov_type3(aov_gamma_emp_se)

write.csv(
  tidy_aov_empse_t3,
  here::here(paste0("results/tidy_aov_empse_t3_", dt, ".csv")))
