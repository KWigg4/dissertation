...
cov_fail <- all_df |> filter(model == "gamma", is.na(est) | is.na(se) | is.na(true))
...
all_df_gamma_w_fails <- all_df %>%
  ...
  dplyr::mutate(
    cover_ind = dplyr::if_else(
      ...
      # KW re ^^ did he mean bias??
      is.na(est) | is.na(se) | is.na(bias),
      NA_real_,
      as.numeric((est - 1.96*se <= true) & (true <= est + 1.96*se))
    ),
...
fn_save_results(summ_grid_gamma)
