...
_v2_kw Changes:  
  ...
  - Didn't use my formulas for part 1 and part 2, so updated those
  - removed 'interaction' variable - don't think we need. 
...
simulate_panel_data <- function(
  ...
  lp1 <- 0.2*post - 0.2*treat + 1.2*female - 0.8*hisp - 0.2*zPCS42
  p_any <- plogis(lp1)
  ...
  lp2 <-7+ 0.2*post - 0.2*treat + 0.4*female - 0.2*hisp - 0.4*zPCS42
  mu <- exp(lp2)
...
compute_ATT_hat <- function(df, m_any, m_pos) {
  ...
  treated_post <- df %>% 
    ...
    # kw added because not in above now
...
compute_true_ATT <- function(df) {
  ...
  # TRUE Part 1
  lp1_obs <- 0.2*treated_post$treat + 0.2*treated_post$post + # 1.0*treated_post$interaction +
    1.2*treated_post$female - 0.8*treated_post$hisp - 0.2*treated_post$zPCS42
  p_obs <- plogis(lp1_obs)

  lp1_cf <- 0.2*treated_post$treat + 0.2*treated_post$post + # 1.0*treated_cf$interaction +
  1.2*treated_post$female - 0.8*treated_post$hisp - 0.2*treated_post$zPCS42
  p_cf <- plogis(lp1_cf)

  # TRUE Part 2
  lp2_obs <- 7 + 0.2*treated_post$treat + 0.2*treated_post$post + # 0.5*treated_post$interaction +
    0.4*treated_post$female - 0.2*treated_post$hisp - 0.4*treated_post$zPCS42
  mu_obs <- exp(lp2_obs)

  lp2_cf <-  7 + 0.2*treated_post$treat + 0.2*treated_post$post + # 0.5*treated_post$interaction +
    0.4*treated_post$female - 0.2*treated_post$hisp - 0.4*treated_post$zPCS42
  mu_cf <- exp(lp2_cf)
...
run_one <- function(n, mag, p_ext, treatment, gamma_shape) {
  ...
  # this needs to be a gamma GLM, not whatever this is? Or is same? but 
  # m_pos <- glm(log(TOTEXPYY) ~ treat*post + female + hisp + zPCS42,
  #              data = df_pos)
  m_pos <- glm(
    TOTEXPYY ~ post*treat + female + hisp + zPCS42,
    data = df_pos,
