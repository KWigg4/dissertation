...
eta_with_p <- function(aov_obj, model_label,
  ...
  # 1) p-values from ANOVA table (works for aov() and car::Anova() once tidied)
  tab_p <- broom::tidy(aov_obj) %>%
    dplyr::filter(!is.na(term), term != "Residuals") %>%
    dplyr::select(term, p.value)

  # 2) effect sizes from effectsize::eta_squared()
  #    NOTE: ci level is passed via 'ci', not 'ci_level'
  ...
  # effectsize names are typically: Parameter, Eta2, CI_low, CI_high
  # We'll standardize/guard against name variations just in case.
  nm <- names(es)
  # Map names safely
  if ("Parameter" %in% nm) names(es)[names(es) == "Parameter"] <- "term"
  if ("Eta2" %in% nm) names(es)[names(es) == "Eta2"] <- "eta2"
  if ("CI_low" %in% nm) names(es)[names(es) == "CI_low"] <- "ci_low"
  if ("CI_high" %in% nm) names(es)[names(es) == "CI_high"] <- "ci_high"
  ...
  out <- dplyr::left_join(es, tab_p, by = "term") %>%
    dplyr::mutate(
      ...
      es_type = switch(type,
        eta2 = "eta^2",
        partial = "partial eta^2",
        generalized = "generalized eta^2"
      ),
