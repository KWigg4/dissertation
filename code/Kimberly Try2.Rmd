...
simulate_panel_data <- function(
  ...
  target_prob = 0.30,       # overall Pr(ind_gt0==1)
  ...
  fill_zero_for_negative = FALSE,            # if TRUE, set TOTEXPYY=0 when ind_gt0==0
  ...
  ind_gt0 <- integer(N)
  if (exact_positives) {
    # exact count to hit 30% (up to rounding)
...
sim1 <- simulate_panel_data(
  ...
  target_prob     = 0.30,
  extreme_alloc   = "proportional",  # or "equal"
  exact_positives = FALSE            # << ensures ~30% overall
)
