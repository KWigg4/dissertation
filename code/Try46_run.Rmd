...
------------------------------------------------------
 ...
 - Baseline Covariates (time-invariant, assigned at individual level), all replicated across both time periods using `rep(., each=T)`  
  ...
  - TPM part 2: gamma distribution with fixed shape = 3, scale adjusted for each observation (gamma has 2 parameterizations: Shape-Rate [`shape` and `rate`] and Shape-Scale [`shape` and `scale` > what this code is using...]). Their relationship is `mean=shape*scale`. In code, for each observation, the shape parameter stays constant (at 3) but scale parameter varies based on that observation's covariates to ensure `E[TOTEXPYY | covariates] = mu = exp(lp_cont)`. All obs will have the same shape (to control distributional form / variance) but different scales (controls the mean). This is how to implement a GLM-style Gamma regression where the mean depends on covariates but the coefficient of variation stays constant.  
...
# ------------------------------------------------------
 ...
 - `mean_adj` calculates mean of all values >q95 and replaces those extreme values with that mean. This allows us to preserve sample size and is less aggressive than topcoding as it allows variation among extremes.  
 - `median_adj` same as `mean_adj` but uses median instead. This is more robust to super-extreme values than `mean_adj`  
...
# ------------------------------------------------------  
Fits a gamma GLM to estimate DiD effect on positive expenditures only. Will return NULL if fewer than 10 positive observations to prevent fitting issues with tiny samples. In simulations, this will handle edge cases gracefully.  
- Link function: `log` - so coefficients are interpretable as *log-rate ratios*  
    - log(E[Y|x]]) = $\beta_0$ + ... 
    OR  
    - E[Y|X] = exp($\beta_0$ + ... )  
...
grid_test <- expand.grid(
  n = c(5000),                    # Just 1 sample size
  prop_extreme = c(0.01),          # Just 1 proportion
  magnitude_level = c(1),          # Just 1 magnitude
  treatment = c("raw", "topcode"), # Just 2 treatments
  stringsAsFactors = FALSE
)

set.seed(2025)
B_test <- 10  # test 10 reps for now

test_df <- pmap_dfr(
  grid_test,
  ~ run_mc(B_test, ..1, ..3, ..2, gamma_shape = 3, ..4, COEF = COEF)
)
...
saveRDS(test_df, file = here::here(paste0("results/test_df_", dt, ".RDS")))
write.csv(test_df, file = here::here(paste0("results/test_df_", dt, ".csv")), row.names = FALSE)
...
# ======================================================
  ...
  - `all_df_gamma` function: Isolates part 2 (gamma) results and creates a coverage indicator: 1 if 95% CI contains true value, 0 otherwise  
  
  - **Factorial ANOVA on gamma bias** (`aov(bias ~ ...)`) tests:  
    - Main effects: Does each factor independently affect bias?  
    - Interactions: Do factors combine to affect bias? (e.g., `treatment:magnitude_level` - does the best treatment change depending on how extreme the outliers are? with 4-way interaction tests to tell if optimal strategy depends on all conditions simultaneously  
    - Unit of Analysis: Each individual replication (67500 rows for gamma)  
    
  - **Factorial ANOVA on gamma SE**  
    - Tests which factors affect **precision** of estimates (e.g., does truncation increase SE more than topcoding?).  
    - Why it matters? Bias-variance tradeoff. One method might reduce bias, but inflate variance (hurting RMSE)   
    
  - **Factorial ANOVA on gamma coverage**  
    - Why scenario-level? Because coverage is a proportion (between 0 and 1); individual reps have coverage = 0 or 1 (binary), which violates ANOVA assumptions. Aggregating to scenario-level gives approximately normal proportions. Each scenario has 500 reps, so coverage estimates are quite stable.  
    - What this tests: Do methods maintain normal coverage (~0.95)? (e.g., does "raw" (with extremes) have poor coverage due to inflated SEs?)  
    - Gold standard: Coverage should be near 0.95; systematic deviations indicate:  
      - <0.90: Underestimating uncertainty (SEs too small)  
      - 0.97: Overestimating uncertainty (SEs too large, or method too conservative)  
  ...
  2) `aov_att_bias`: Tests if ATT bias differs across conditions. This is more relevant because it shows which methods better recover truth. It's a direct test of RQ4, "Which treatment method produces least biased ATT estimates?"  
  
**Considerations**  

 - ANOVA assumes homoscedasticity: might want to check residual plots  
 
**Next steps to consider**  

 - post-hoc tests (Tukey HSD) to rank treatment methods  
 - Effect size estimates (eta-squared) to quantify importance  
...
# Bv2) factorial ANOVA on gamma emp SE  (KW, 01/29/26)
...
## could test ratio to see if model SE's are calibrated (how well model's claimed uncertainty matches actual uncertainty)? 
summ_grid_gamma <- summ_grid_gamma |> 
	mutate(se_ratio = mean_se / emp_se) 
	# if < 1 then model underestimates uncertainty
  # if > 1 then model overestimates uncertainty
  # If model says SE is 0.03 but estimates vary by SD = 0.09, 0.03/0.09 = 0.33 so under calibrated (CIs too narrow / false precision, Coverage <0.95 (too many CI's miss true value), Type 1 error inflation (will reject null too often), estimates will look more precise than they are. 
