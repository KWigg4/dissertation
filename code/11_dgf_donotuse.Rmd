...
## NOTE: WAS 'Kimberly Try2_kw.Rmd', which was adapted from Felix's original
...
```{r}
sigma_part2 <- sigma(emp_model$model_part2) # residual Stddev (sigma) 6.733
```  
...
simulate_panel_data <- function(
  ...
  sigma_e = 0.4,            # residual SD for log(TOTEXPYY)
  sigma_e = 6.733036,
  # rho = 0.8,                # pre/post corr for PCS42
  rho=0.52,
  ...
  exact_positives = TRUE           # if TRUE, hit 30% exactly (up to rounding)
) {
  ...
  # ----- Part 1: Binary outcome (calibrate to ~70% overall with extremes forced positive) -----
  # Linear predictor WITHOUT intercept on non-extreme rows
  # lp_cov <- 0.5*ind_post + 0.7*ind_tx + 0.2*ind_female - 0.2*HISPANX + 0.2*INSCOV + 0.1*zPCS + u
  ...
  if (!is.finite(target_rest) || target_rest <= 0 || target_rest >= 1) {
    warning(sprintf("prop_extreme=%.3f with target_prob=%.3f => target_rest=%.3f; clamping.", prop_extreme, target_prob, target_rest))
  ...
  #            0.2*ind_female + 0.1*HISPANX + 0.2*INSCOV + u
...
# ============================
set.seed(20250811)
all_sim_results_noExtreme <- vector("list", nrow(sim_conditions))
...
for (i in seq_len(nrow(sim_conditions))) {
  ...
  all_sim_results_noExtreme[[i]] <- sim_df
}

final_sim_data_noExtreme <- bind_rows(all_sim_results_noExtreme)
...
# 1) Did each scenario hit ~70% positives and the correct extreme share?
...
pass_pos  <- (abs(K/N - 0.70) <= 0.001)              # ~30% exactly
...
saveRDS(
  final_sim_data,
  here::here("data_processed/final_sim_data.RDS")
  )

saveRDS(
  final_sim_data_noExtreme,
  here::here("data_processed/final_sim_data_noExtreme.RDS")
)
