...
## Pilot study
pilot <- summ_grid |> 
  dplyr::filter(model=="gamma",n==1000, prop_extreme==0.01, magnitude_level ==1)
...
saveRDS(pilot, here::here("data_processed/pilot.RDS"))
...
write.csv(
	pilot_bias_data,
	here::here(paste0("data_results/pilot_allDF",dt,".csv"))
	)
saveRDS(
	pilot_bias_data, 
	here::here(paste0("data_results/pilot_allDF",dt,".RDS"))
	)
...
# bias_anova <- aov(
# 	est ~ model * treatment * magnitude_level * prop_extreme * n, data = all_df)
# summary(bias_anova)
# 
# anova_gamma <- aov(est ~ treatment * magnitude_level * prop_extreme * n, data = filter(all_df, model == "gamma"))
# summary(anova_gamma)
